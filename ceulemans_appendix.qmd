...
function splitbycomma(array) {
  ...
  array.forEach(value => {
    const words = [];
    if (value === null) {} else if (value.includes(",")) {words.push(value.split(","));} else if (value.includes("/")) {words.push(value.split("/"));} else if (value.includes(";")) {words.push(value.split(";"));} else {words.push(value);}
...
viewof attribution = Inputs.select(newauthor.sort((a, b) => a.localeCompare(b)), {multiple: false, value: "Hesychios"})
...
filtered_ref = data.filter(function(pls) {
  if (pls["Attribution"]) {return pls["Attribution"].includes(attribution);}
  else if (pls["Possible Source"]) {return pls["Possible Source"].includes(attribution);}
})
...
The source code of this website and the dataset used is at <https://github.com/bibelexegese/ceulemans_appendix> [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.XXXXXXX.svg)](https://doi.org/10.5281/zenodo.XXXXXXX).

## Cite as
Reinhart Ceulemans, The *hermēneiai* published in *Les chaînes exégétiques grecques sur les Psaumes* (1986–2018) – Dataset, v0.1 (beta), 2024, <https://nbn-resolving.org/urn:nbn:de:kobv:b4-20240508094043722-4354412-8>.
