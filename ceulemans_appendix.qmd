...
```{ojs}
//| output: false
all_attrib = data.map(row => {
  const attribution = row.Attribution || '';
  const possibleSource = row["Possible Source"] || '';
  if (attribution) {
    return attribution + (possibleSource ? `,${possibleSource}` : '');
  } else if (possibleSource) {
...
filtered_ref = data.filter((pls) => {
  ...
  let possibleSourceMatch = false;

  if (pls["Attribution"] !== null && pls["Attribution"] !== undefined) {
    if (pls["Attribution"].includes(",")) {
      attributionMatch = pls["Attribution"].split(",").some(word => word.trim() === attribution);
    } else {
      attributionMatch = pls["Attribution"].trim() === attribution;
    }
  }

  if (pls["Possible Source"] !== null && pls["Possible Source"] !== undefined) {
    if (pls["Possible Source"].includes(",")) {
      possibleSourceMatch = pls["Possible Source"].split(",").some(source => source.trim() === attribution);
    } else {
      possibleSourceMatch = pls["Possible Source"].trim() === attribution;
    }
  }

  return attributionMatch || possibleSourceMatch;
});
